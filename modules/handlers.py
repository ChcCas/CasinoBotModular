...
# ——— Константи ———————————————————————————————
# PROJECT_ROOT — це папка src/, з неї потрібно відкрити welcome.gif
...
def setup_handlers(app):
    ...
    # Адмін-reply: пересилка відповідей адміну користувачеві
    app.add_handler(
...
async def back_to_confirm_file(update, context):
    await update.callback_query.message.reply_text("Надішліть підтвердження:", reply_markup=nav_buttons())
...
async def menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "admin_deposits":
        # … існуюча логіка Admin → Deposits …
        return STEP_MENU
    if data == "admin_users":
        # … існуюча логіка Admin → Users …
        return STEP_MENU
    if data == "admin_withdrawals":
        # … існуюча логіка Admin → Withdrawals …
        return STEP_MENU
    if data == "admin_stats":
        # … існуюча логіка Admin → Stats …
        return STEP_MENU
...
async def client_menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Якщо вже авторизований — показуємо авторизоване меню
    with sqlite3.connect(DB_NAME) as conn:
        auth = conn.execute(
...
async def show_authorized_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.effective_chat.send_message(
        "Вітаємо в особистому профілі!", reply_markup=InlineKeyboardMarkup(kb)
    )
...
async def authorized_menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "cashback":
        await q.message.reply_text("Сценарій зняття кешбеку…")
        return STEP_CLIENT_AUTH
    ...
    if data == "history":
        await q.message.reply_text("Тут буде історія ваших операцій…")
        return STEP_CLIENT_AUTH
...
async def profile_enter_phone(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = await context.bot.send_message(
        ADMIN_ID, f"Хоче дізнатися картку — телефон: {phone}"
    )
    with sqlite3.connect(DB_NAME) as conn:
        conn.execute(
...
async def profile_enter_code(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    uid = update.effective_user.id
    msg = await context.bot.send_message(ADMIN_ID, f"Код для {uid}: {code}")
    with sqlite3.connect(DB_NAME) as conn:
        conn.execute(
            "INSERT OR REPLACE INTO threads(admin_msg_id,user_id) VALUES(?,?)",
            (msg.message_id, uid)
...
async def register_code(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    uid  = update.effective_user.id
    msg = await context.bot.send_message(
        ADMIN_ID, f"Код реєстрації {code} від {name} ({uid})"
    )
    with sqlite3.connect(DB_NAME) as conn:
        conn.execute(
            "INSERT OR REPLACE INTO threads(admin_msg_id,user_id) VALUES(?,?)",
...
async def admin_broadcast(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for uid in recipients:
        try:
            ...
            pass
    await update.message.reply_text(f"Розсилка надіслана {sent} користувачам.", reply_markup=nav_buttons())
    return STEP_MENU
...
async def admin_search(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = update.message.text.strip()
    if not text.isdigit():
        await update.message.reply_text("Введіть числовий ID користувача.", reply_markup=nav_buttons())
        return STEP_ADMIN_SEARCH
    uid = int(text)
    with sqlite3.connect(DB_NAME) as conn:
    ...
    parts = [f"🆔 ID: {uid}"]
    if client:
        parts.append(f"📇 Клієнт: {client[0]} / {client[1]} (auth={bool(client[2])})")
    else:
    ...
    if deps:
        for r in deps:
            parts.append(f"  • {r[4]} — {r[0]}, {r[1]}, {r[2]}, сума {r[3]}")
    else:
        parts.append("  — немає")
    parts.append("\n💸 Виведення:")
    if wds:
...
async def admin_reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # якщо клієнт не авторизований — reply адміну = номер картки
